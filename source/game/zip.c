...
u32 ZipExtractContent(const char* path, const char* extrpath, u32 CO, u32* newCO, char* ErrorDesc, u32* flags){
    ...
    */
    ...
    bool overwrite = (flags && (*flags & OVERWRITE_ALL));
    u32 ArchiveSize = FileGetSize(path);
    u32 FileNameLength = 0; //File name length
    u32 FileSize = 0;       //FileSize
    u32 FileCommentSize = 0; // Length of comment field
    u32 ErrorCode= 0; // temporary error code  (used when error number 3 or 4 occurred)
    u64 FileDataStart_o; // start offset of adtually data
    u64 FileDataEnd_o; // end offset of adtually data
    u32 Temp = 0;
    u32 Temp1 = 0;
    ...
    Temp = 0;
    Temp1 = 0;
    if (FileGetData(path, &Temp, 0x04, CO + 0x12) == 0) return 10; //get compressed file size
    if (FileGetData(path, &Temp1, 0x04, CO + 0x16) == 0) return 11; //get original file size
    if (Temp != Temp1){ // if not compressed, they are same
    ...
    FileSize = Temp; // pick compressed one to calculate correct newCO(new current offset) on error
    
    // Get File Name Length
    Temp = 0;
    if (FileGetData(path, &Temp, 0x02, CO + 0x1A) == 0) return 12;
    FileNameLength = Temp;
    ...
    // Get comment size
    Temp = 0;
    if (FileGetData(path, &Temp, 0x02, CO + 0x1C) == 0) return 13;
    FileCommentSize = Temp;
    ...
    Temp = 0;
    if (FileGetData(path, &Temp, 0x01, CO + 0x04) == 0) return 15;
    if (Temp == 0x0a){
