...
void ConfigMenu() {
	...
	// back up old config
	int screen_brightness_old = screen_brightness;
	bool show_space_old = show_space;
	bool use_jpn_clock_old = use_jpn_clock;
	char font_path_old[256];
	snprintf(font_path_old, 255, "%s", font_path);
    ...
    while (true) {
        ...
        if (pad_state & BUTTON_DOWN) sel = (sel+1) % n_opt;
			...
			if (SaveConfig()) ShowPrompt(false, "All settings saved");
